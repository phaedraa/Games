...
class Memory:
    ...
    def __init__(self, num_cards):
        ...
        self.deck_size = num_cards # deck_size must be an even integer, [8, 20]
        self.BOARD_WIDTH = 50 * self.deck_size
        self.BOARD_HEIGHT = 100
        
        # create frame and add a button and labels
        self.frame = simplegui.create_frame("Memory", self.BOARD_WIDTH, 
            self.BOARD_HEIGHT)
    ...
    def new_game(self):
        ...
        self.exposed = [False for card in range(self.deck_size)]
    ...
    def expose(self):
        if self.mouse_pos[1] >= 0 and self.mouse_pos[1] <= self.BOARD_HEIGHT:
            ...
            card = 0
            found = False
            while x_pos_2 <= self.BOARD_WIDTH and not found:
                if self.exposed[card] != True:
                    if self.mouse_pos[0] >= x_pos_1 and \
                    self.mouse_pos[0] <= x_pos_2:
    ...
    def draw(self, canvas):
        ...
        for idx, card in enumerate(self.deck):
            if self.exposed[idx] == True:
                canvas.draw_polygon([
                    (x_pos_2, self.BOARD_HEIGHT), (x_pos_1, self.BOARD_HEIGHT), 
                    (x_pos_1,0), (x_pos_2,0)], 3, 'Black', 'Red')
                canvas.draw_text(str(card), 
                    (x_pos_1 + (self.BOARD_WIDTH / self.deck_size) / 5, 
                    0.7 * self.BOARD_HEIGHT), 0.6 * self.BOARD_HEIGHT, 'Black')
            else:
                canvas.draw_polygon([
                    (x_pos_2, self.BOARD_HEIGHT), (x_pos_1, self.BOARD_HEIGHT), 
                    (x_pos_1,0), (x_pos_2,0)], 3, 'Black', 'Green')
            x_pos_1 += self.BOARD_WIDTH / self.deck_size
            x_pos_2 += self.BOARD_WIDTH / self.deck_size
        self.label.set_text("Total turns: " + str(self.turns))  
